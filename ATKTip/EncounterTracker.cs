...
public sealed class EncounterTracker : IDisposable
{
    ...
    public void RebuildZoneMappings()
    {
        ...
        // Also map EncounterId=0 custom timelines by encounter name using synthetic negative IDs.
        // This allows them to trigger the overlay even when no fetched FFLogs report exists.
        var syntheticId = -1;
        ...
        {
            // Only add if not already covered by a real encounter ID with the same name.
        ...
        {
            log.Debug("EncounterTracker: no encounters to map (fetched, custom, or name-based), skipping zone map build.");
            return;
        }
