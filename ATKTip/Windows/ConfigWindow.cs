...
public sealed class ConfigWindow : Window
{
    ...
    private void DrawOverlayAppearance()
    {
        ...
        {
            cfg.OverlayShowFreqBars = showFreqBars;
            changed = true;
        }
        Tooltip("Show a small bar under icons indicating usage frequency.\nOnly shown when frequency is below 95%.");

        var showTimer = cfg.OverlayShowTimer;
        if (ImGui.Checkbox("Show Timer", ref showTimer))
        {
            cfg.OverlayShowTimer = showTimer;
            changed = true;
        }
        Tooltip("Show the elapsed combat time in the overlay header.");
