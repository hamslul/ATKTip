...
public sealed class MainWindow : Window
{
    ...
    public override void Draw()
    {
        ...
        if (ImGui.BeginTabItem("Config", configFlags))
        {
            DrawConfigTab();
            ImGui.EndTabItem();
        }
    ...
    private void ExportAllTimelines()
    {
        var customs = plugin.Configuration.CustomTimelines;
        if (customs.Count == 0) { SetEiStatus("Nothing to export.", true); return; }
        try
        {
            var json = Newtonsoft.Json.JsonConvert.SerializeObject(customs, Newtonsoft.Json.Formatting.Indented);
            ImGui.SetClipboardText(json);
            SetEiStatus($"Exported {customs.Count} timeline(s) to clipboard.");
        }
        catch (Exception ex) { SetEiStatus($"Export failed: {ex.Message}", true); }
    }

    private void ExportSingleTimeline(string key, AggregatedTimeline tl)
    {
        try
        {
            var wrapper = new Dictionary<string, AggregatedTimeline> { [key] = tl };
            var json = Newtonsoft.Json.JsonConvert.SerializeObject(wrapper, Newtonsoft.Json.Formatting.Indented);
            ImGui.SetClipboardText(json);
            SetEiStatus($"Exported \"{tl.EncounterName} / {tl.SpecName}\" to clipboard.");
        }
        catch (Exception ex) { SetEiStatus($"Export failed: {ex.Message}", true); }
    }

    private void ImportTimelinesFromClipboard()
    {
        try
        {
            ...
            var incoming = Newtonsoft.Json.JsonConvert.DeserializeObject<Dictionary<string, AggregatedTimeline>>(text);
            if (incoming == null || incoming.Count == 0) { SetEiStatus("No timelines found in clipboard.", true); return; }

            var customs  = plugin.Configuration.CustomTimelines;
            var added    = 0;
            var replaced = 0;
            foreach (var (key, tl) in incoming)
            {
                if (customs.ContainsKey(key)) replaced++;
                else added++;
                customs[key] = tl;
            }
            plugin.SaveConfig();
            SetEiStatus($"Imported {added} new, {replaced} updated.");
        }
    ...
    private void DrawCustomTimelinesTab()
    {
        ...
        ImGui.Separator();
        if (ImGui.Button("Export All", new Vector2(-1, 0)))
            ExportAllTimelines();
        if (ImGui.IsItemHovered())
        {
            ImGui.BeginTooltip();
            ImGui.Text("Copy all custom timelines to clipboard as JSON.");
            ImGui.EndTooltip();
        }
        if (ImGui.Button("Import", new Vector2(-1, 0)))
            ImportTimelinesFromClipboard();
        if (ImGui.IsItemHovered())
        {
            ImGui.BeginTooltip();
            ImGui.Text("Paste custom timelines from clipboard JSON.\nExisting timelines with the same key are replaced.");
            ImGui.EndTooltip();
        }
        ...
        if (ImGui.SmallButton("Export"))
            ExportSingleTimeline(selectedCustomKey!, editingTimeline);
        if (ImGui.IsItemHovered())
        {
            ImGui.BeginTooltip();
            ImGui.Text("Copy this timeline to clipboard as JSON.");
            ImGui.EndTooltip();
        }
        ImGui.SameLine();
            ...
            new Vector2(0, -1)))
        {
            ...
            if (ImGui.BeginPopup("##EditEntry") && editingEntryIndex >= 0)
            {
                ...
                else if (!editingEntryIsBoss && editingEntryIndex < playerEntries.Count)
                {
                    ...
                    ImGui.SetNextItemWidth(110);
                    ImGui.InputFloat("Time (s)##ptime", ref editEntryTime, 0.1f, 1f, "%.2f");
                    ImGui.SetNextItemWidth(110);
    ...
    private void DrawSelectors()
    {
        ...
        if (ImGui.IsItemHovered())
        {
            ...
            ImGui.Text("Use Update Logs to fetch fresh data from FFLogs.");
            ImGui.EndTooltip();
        }

        ImGui.SameLine();

        // Clear button — clears from local cache and display
        ...
        {
            ImGui.BeginTooltip();
            ImGui.Text("Clear the cached data for the selected fight/job from disk.");
            ImGui.EndTooltip();
        }

        // Update Logs + Preview Overlay on the same row
        var canUpdate = !isUpdating && selectorsLoaded && zones.Count > 0 && allSpecNames.Count > 0;
        if (!canUpdate) ImGui.BeginDisabled();
        if (ImGui.Button("Update Logs", default))
        {
            StartUpdate();
        }
        ...
        if (ImGui.Button("Preview Overlay", default) && hasTimeline)
        {
        ...
        if (ImGui.IsItemHovered())
        {
            ImGui.BeginTooltip();
            if (hasTimeline)
                ImGui.Text("Preview the live overlay window with the current timeline.");
            else
    ...
    private void DrawCfgOverlaySettings()
    {
        ...
        var overlayScale = cfg.OverlayScale;
        ImGui.SetNextItemWidth(200);
        if (ImGui.SliderFloat("Overlay Scale", ref overlayScale, 0.5f, 2.0f, "%.1f"))
        {
            cfg.OverlayScale = overlayScale;
            plugin.SaveConfig();
        }
    }
    ...
    private void DrawCfgOverlayAppearance()
    {
        ...
        var showFreqBars = cfg.OverlayShowFreqBars;
        if (ImGui.Checkbox("Show Frequency Bars", ref showFreqBars))
        { cfg.OverlayShowFreqBars = showFreqBars; changed = true; }
        CfgTooltip("Show a small bar under icons indicating usage frequency.\nOnly shown when frequency is below 95%.");

        var showTimer = cfg.OverlayShowTimer;
        if (ImGui.Checkbox("Show Timer", ref showTimer))
        { cfg.OverlayShowTimer = showTimer; changed = true; }
        CfgTooltip("Show the elapsed combat time in the overlay header.");
    ...
    private void DrawCfgAntsSettings()
    {
        ...
        if (cfg.AntsEnabled)
        {
            ...
            if (cfg.AntsCustomEnabled)
            {
                ...
                CfgTooltip("Colour of the marching dashes.");

                var gapCol = cfg.AntsGapColor;
                if (ImGui.ColorEdit4("Gap colour##ants", ref gapCol,
                    ImGuiColorEditFlags.AlphaBar | ImGuiColorEditFlags.AlphaPreviewHalf))
                { cfg.AntsGapColor = gapCol; changed = true; }
                CfgTooltip("Colour drawn in the gaps between dashes. Set alpha to 0 for transparent gaps.");
    ...
    private void DrawCfgResetSection()
    {
        ...
        if (ImGui.Button("Reset to Defaults", default))
        {
            ...
            cfg.OverlayScale           = 1.0f;
            cfg.OverlayPixelsPerSec    = 60.0f;
            cfg.OverlayIconSize        = 32.0f;
            cfg.OverlayTimeBehind      = 4.5f;
            cfg.OverlayBgOpacity       = 0.0f;
            cfg.OverlayPastAlpha       = 1.0f;
            cfg.OverlayFreqThreshold   = 0.40f;
            cfg.OverlayShowGrid        = true;
            cfg.OverlayShowFreqBars    = true;
            cfg.OverlayShowTimer       = true;
