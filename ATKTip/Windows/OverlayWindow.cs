...
public sealed class OverlayWindow : Window, IDisposable
{
    ...
    public OverlayWindow(Plugin plugin, ICondition condition, IDutyState dutyState, IPluginLog log)
        : base("ATKTip##Overlay",
    ...
    /// <summary>
    /// Returns true when <paramref name="abilityId"/> is at the single "active" ants
    /// timestamp — the earliest timeline entry currently inside the configured window
    /// around the now-line that also meets the frequency threshold.
    /// Only one timestamp is active at a time, so abilities are highlighted
    ...
    public bool IsAbilityInAntsWindow(int abilityId)
    {
        ...
        // Convert to seconds so ants fire when the icon visually crosses the red line.
        ...
        if (IsOGCD(abilityId))
        {
            // oGCDs: every entry for this ability that falls inside the ants window glows.
            // No single-winner restriction — multiple oGCDs (and GCDs) can glow at once.
        ...
        // GCDs: exactly one glows at a time — the GCD entry closest to the red line.
        Data.TimelineEntry? bestGcd = null;
        var minAbsRel = double.MaxValue;
        foreach (var e in activeTimeline.Entries)
        {
            if (e.Frequency < GetAbilityThreshold(e.AbilityId)) continue;
            if (IsOGCD(e.AbilityId)) continue;
    ...
    /// <summary>
    /// Returns the set of all ability IDs that should display ants this frame.
    /// Called on the UI thread by <see cref="AntsController"/>.
    /// </summary>
    public HashSet<int> GetAntsAbilityIds()
    {
        var result = new HashSet<int>();
        if (!Cfg.AntsEnabled) return result;
        if (activeTimeline == null) return result;
        if (!inCombat && !isPreview) return result;
        ...
        // All oGCDs in window glow simultaneously
        foreach (var e in activeTimeline.Entries)
        {
            if (e.Frequency < GetAbilityThreshold(e.AbilityId)) continue;
            if (!IsOGCD(e.AbilityId)) continue;
            var rel = (e.TimeOffsetSec + ogcdTimeDelta) - elapsed;
            if (rel >= -after && rel <= before) result.Add(e.AbilityId);
        }

        // Exactly one GCD — the closest to the now-line
        Data.TimelineEntry? bestGcd = null;
        var minAbsRel = double.MaxValue;
        foreach (var e in activeTimeline.Entries)
        {
        ...
        if (bestGcd != null) result.Add(bestGcd.AbilityId);
    ...
    public override void PreDraw()
    {
        ...
        // Allow mouse input for preview scrubbing or when unlocked (move/resize).
        // Keep NoInputs in locked combat mode so the overlay doesn't steal focus.
        if (isPreview || !Cfg.OverlayLocked)
    ...
    public override void Draw()
    {
        ...
        // ── Scrub bar (preview only) ──────────────────────────────────
        // Row: [▶/⏸] [🔒] [track...........] [time] [×]
        var scrubH = 0.0f;
        if (isPreview && fightDur > 0)
        {
            ...
            if (ImGui.IsMouseHoveringRect(barTL, barBR) && ImGui.IsMouseClicked(ImGuiMouseButton.Left))
                isScrubbing = true;
            ...
            if (isScrubbing)
            {
                if (ImGui.IsMouseDown(ImGuiMouseButton.Left))
                {
                    ...
                    previewManualTimeSec = mfrac * fightDur;
                    combatElapsedSec     = previewManualTimeSec;
                }
                else
                {
            ...
            if (ImGui.IsWindowHovered())
            {
                ...
                {
                    previewAutoplay      = false;
                    previewManualTimeSec = Math.Clamp(previewManualTimeSec - wheel * 2.0, 0.0, fightDur);
                    combatElapsedSec     = previewManualTimeSec;
                }
            }
        ...
        try
        {
            ...
            if (sheet != null)
            {
                ...
                if (row.HasValue)
                {
                    ...
                    // ActionCategory in Lumina:
                    //   1 = AutoAttack
                    //   2 = Spell      (GCD)
                    //   3 = Weaponskill (GCD)
                    //   4 = Ability    (oGCD)  ← what we want
                    // Fallback: if CooldownGroup != 58, it has its own recast → oGCD
                    if (cat == 4)
                        result = true;
                    else if (cat != 2 && cat != 3 && cat != 1)
                        result = row.Value.CooldownGroup != 58;
                }
            }
        }
        catch { }

        oGcdCache.TryAdd(abilityId, result);
        return result;
    }
