...
public sealed class TimelineAggregator
{
    ...
    public List<BossTimelineEntry> AggregateBossEvents(List<RawBossCastEvent> events)
    {
        ...
        // ── Deduplicate multi-hit entries ─────────────────────────────────────
        // Many boss abilities hit multiple targets in a tight burst, producing
        // several cast events for the same ability within fractions of a second.
        // Walk the sorted list and drop any entry whose AbilityId already appeared
        // within the last DeduplicateWindowSec — keeping only the earliest hit.
        const double DeduplicateWindowSec = 1.5;
        var deduped = new List<BossTimelineEntry>(result.Count);
        // lastSeen[abilityId] = CastStartSec of the most recently kept entry for that ability
        ...
        foreach (var entry in result)
        {
            ...
            {
                // This is a repeat hit of the same ability within the window — drop it.
                continue;
            }
