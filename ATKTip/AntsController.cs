...
/// combo/proc highlight.  When <see cref="Configuration.AntsCustomEnabled"/> is on, the
/// native ants are suppressed for ATKTip-managed abilities and replaced by a fully
/// customisable ImGui-drawn dashed border rendered on top of the hotbar slots.
/// </summary>
public sealed unsafe class AntsController : IDisposable
{
    ...
    private bool HandleIsActionHighlighted(
        ...
        uint           actionId)
    {
        // Always run the original first — never suppress a highlight the game wants.
        ...
        var inAnts = plugin.OverlayWindow.IsAbilityInAntsWindow((int)actionId);

        // When custom ants are enabled we draw our own border, so suppress the native one.
        if (inAnts && plugin.Configuration.AntsCustomEnabled)
            return false;
    ...
    public void Draw()
    {
        ...
        var antsIds = plugin.OverlayWindow.GetAntsAbilityIds();
        if (antsIds.Count == 0) return;
        ...
        var padding = cfg.AntsBorderPadding - 10f;
        ...
        for (var barIdx = 0; barIdx < HotbarAddonNames.Length; barIdx++)
        {
            ...
            for (var slotIdx = 0; slotIdx < 12; slotIdx++)
            {
                ...
                if (!antsIds.Contains((int)slot->CommandId)) continue;
                ...
                DrawMarchingAnts(x, y, w, h, time, cfg);
            }
        }
    }
        ...
        float time, Configuration cfg)
    {
        ...
        var dashCol = ImGui.ColorConvertFloat4ToU32(cfg.AntsColor);
        var gapCol  = ImGui.ColorConvertFloat4ToU32(cfg.AntsGapColor);
