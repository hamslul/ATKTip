...
public sealed unsafe class AntsController : IDisposable
{
        ...
        uint           actionId)
    {
        ...
        // When custom ants mode is on, ATKTip draws its own border for managed abilities.
        // Only suppress native highlights for abilities ATKTip is managing; let native
        // combo/proc ants through for everything else (e.g. standard combo chains).
        if (cfg.AntsEnabled && cfg.AntsCustomEnabled)
        {
            var inAnts = plugin.OverlayWindow.IsAbilityInAntsWindow((int)actionId);
            if (inAnts) return false;
            return original;
        }
